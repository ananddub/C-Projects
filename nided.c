...
void insert(node*head,char data[1000])
        {
        node *move=head;
        if(move->next!=NULL)
                {
...
void boxc(node*pr,node*move)
        ...
        if(pr!=NULL)
                ...
                int len;
                if(pr==move)bk(BLUE); 
                len=printf("\t\t\t   %s",pr->data);
                for(;len<10;len++)printf(" ");
                printf("\n");
                // if(pr->next==NULL)last=pr; 
                pr=pr->next;
                bk(0);
                boxc(pr,move);
                }
...
void linkprint(node*head)
        ...
        while(1)
                ...
                switch(getch())
                        {
                        case 'A':
                                 if(move->prev!=NULL)move=move->prev ;
                                 else move=last;  
                                 break;
                        case 'B':
                                if(move->next!=NULL)move=move->next;
                                else move=head;
                                break; 
