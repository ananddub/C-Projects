...
int main(int argc, char *argv[])
        ...
        if (strchr(filename, '/') != NULL)
                ...
                strcpy(data, tempc);
                if (strstr(tempc, ".c") != NULL)
                        {
                        len = strlen(data) - 2;
                        tempc[len] = '\0';
                        }
                sprintf(exec, "cd %s && clear && gcc -Wall -Wextra  %s.c  -lncurses ", cd, tempc, tempc, tempc);
                system(exec);
                }
        else
                {
                sprintf(data, "clear && gcc -Wall -Wextra  %s.c  -lncurses", tempc, tempc, tempc);
                system(data);
                }
        printf("\n");
        while (1)
